export function analyzePortfolioPrompt(user : any) {
    ...
    let postPrompt = "based on the provided information. Give us a response which analyzes this portfolio and give actions that should be taken to decrease carbon emission. give a very structured and to the point response eliminating every line that is not necessary. the response should only contain what has been asked. try to have specific reports. Make sure the suggestions are realistic and are easy to implement. The response type should be concise and should not use unnecessary lengthy lines. do not write any of the input again. I just want the analysis in the response and nothing else. your answer should be a plain string with alphabet and numbers only, do not use * or anything like that."
